...
def preprocess(image, img_size, stride=32, pad=0.5):
    # Padded resize
...
def inference_yolo(image, model, img_size, conf_thres, iou_thres):
    ...
    outputs = model.run(None, {model.get_inputs()[0].name: np.array(batch)})
    out = torch.from_numpy(np.array(outputs[0]))
    out = non_max_suppression(out, conf_thres=conf_thres, iou_thres=iou_thres, multi_label=False)

    out[0][:, :4] = scale_coords((w, h), out[0][:, :4], (w0, h0)).round()
