...
from lib.config.settings import ONNX_CARD_DETECTION_MODEL_PATH
...
class CardDetection(object):
    ...
    def detect_corners(self, img):
        ...
        if img is None:
            return []
        target = inference_yolo(img, self.card_model, img_size=IMG_SIZE, conf_thres=CONF_THRES, iou_thres=IOU_THRES)
        if target is None:
            return []
